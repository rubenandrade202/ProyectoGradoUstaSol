...
nsVen.UpdateBilleteSumarieGrid = function (idCaja, valorBillete) {
    var bufferSelectorGrid = $("#gmVenta_" + idCaja);
    var bufferSelectorSpan = $("#cambioVenta_" + idCaja);
    var bufferSelectorBillete = $("#txtBilleteVenta_" + idCaja);
    var cambio = 0;
    var billete = 0;
    if (valorBillete == null || valorBillete == undefined) {
        billete = bufferSelectorBillete.igCurrencyEditor("value");
    } else {
        billete = valorBillete;
    }

    billete = (billete == null || billete == undefined) ? 0 : billete;

    var totalVenta = bufferSelectorGrid.igGridSummaries("summariesFor", "Subtotal")[0].result;
    totalVenta = (totalVenta == null || totalVenta == undefined) ? 0 : totalVenta;

    cambio = billete - totalVenta;

    if (cambio >= 0) {
        bufferSelectorSpan.css("color", "LimeGreen");
    } else {
        bufferSelectorSpan.css("color", "red");
    }
...
nsVen.AddItemToGrid = function (idCaja, idProducto, precioProducto, nombreProducto) {    
    ...
    nsVen.UpdateBilleteSumarieGrid(idCaja);
}
...
nsVen.ModificarQtyRowGrid = function (idGrid, idProducto, increaseDecrease) {
    ...
    nsVen.UpdateBilleteSumarieGrid(idGrid, null);
}
...
$(".click-Generic-Sale").click(function () {
    ...
    nsVen.UpdateBilleteSumarieGrid(caja, null);
});
//#endregion
